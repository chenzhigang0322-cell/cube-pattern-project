...
import { ETHNIC_PATTERN_CONFIGS, PatternType } from '@/types/pattern'
import { usePatternStore } from '@/store/pattern-store'
import { Card, CardContent } from '@/components/ui/card'
import { cn } from '@/lib/utils'
import { Flower2, Star } from 'lucide-react'

interface EthnicPatternSelectorProps {
  onPatternSelect?: (type: PatternType) => void
}

export function EthnicPatternSelector({ onPatternSelect }: EthnicPatternSelectorProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium text-amber-100">少数民族纹样</h3>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {ETHNIC_PATTERN_CONFIGS.map((config) => (
          <Card
            key={config.type}
            className={cn(
